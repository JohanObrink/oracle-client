...
class OracleSocketServer {
  ...
  onSocketConnection(socket) {
    ...
    var onDbConnect = (config, callback) => this.onDbConnect(socket, config, callback);
    var onDbExecute = (connectionId, sql, parameters, callback) => this.onDbExecute(socket, connectionId, sql, parameters, callback);
    var onDbRelease = (connectionId, callback) => this.onDbRelease(socket, connectionId, callback);
    var onSocketDisconnect = () => {
      socket.off('db-connect', onDbConnect);
      socket.off('db-execute', onDbExecute);
      socket.off('db-release', onDbRelease);
  ...
  onDbExecute(socket, connectionId, sql, parameters, callback) {
    var connection = socket.connections[connectionId];
    if(!connection) {
